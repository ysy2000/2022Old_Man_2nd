...
def train( train_name, model, data_loader, criterion, optimizer, scheduler, device, epoch, train_sampler, max_norm=400, teacher_forcing_ratio=1):
    ...
    for i, (data) in enumerate(data_loader):
        ...
        ##################LOG######################################################
        with open("/home/ysy/2022Old_Man_2nd/log/"+train_name+"_TrainLog.txt", "a") as f:
            f.write('Epoch %d batch %d/%d Loss %0.4f CER %0.4f\n' %((epoch + 1), (i + 1), len(train_sampler), loss, cer))
...
def evaluate(train_name,model, data_loader, criterion, device, save_output=False):
    ...
    with torch.no_grad():
        ...
        for i, (data) in enumerate(data_loader):
            ...
            total_dist += dist
            total_length += length
            if save_output and (i == 1) :
                #transcripts_list += transcripts 
                ##################LOG######################################################
                with open("/home/ysy/2022Old_Man_2nd/log/"+train_name+"_InfLog.txt", "a") as f:
...
def main():
    ...
    parser.add_argument('--train-file', type=str,
                        help='data list about train dataset', default='/home/ysy/2022Old_Man_2nd/data_list_train.csv') # val 일정 비율 떼어서 사용하기
    parser.add_argument('--val-file-list', nargs='*',
                        help='data list about test dataset', default='/home/ysy/2022Old_Man_2nd/data_list_val.csv')


    parser.add_argument('--labels-path', default='/home/ysy/2022Old_Man_2nd/labels_for_chars.csv', help='Contains large characters over korean')# 음절 리스트 for token
    parser.add_argument('--dataset-path', default='/home/ysy/old_man_data/', help='Target dataset path')
    ...
    parser.add_argument('--batch_size', type=int, default=80, help='Batch size in training (default: 32)')
    ...
    # System    
    ## 경로 설정 필요 
    parser.add_argument('--save-folder', default='/home/ysy/2022Old_Man_2nd/saved_models', help='Location to save epoch models')
    ...
    enc = EncoderRNN(
                     input_size, args.encoder_size, n_layers=args.encoder_layers,
                     dropout_p=args.dropout, bidirectional=args.bidirectional, 
                     rnn_cell=args.rnn_type, variable_lengths=False)

    dec = DecoderRNN(
                    len(char2index), args.max_len, args.decoder_size, args.encoder_size,
                     SOS_token, EOS_token,
    ...
    for epoch in range(begin_epoch, args.epochs):
        ...
        #tf = teacheForcing_schdeuler(epoch)

        # Train
        train_loss, train_cer = train( args.train_name,train_model, train_loader, criterion, optimizer, scheduler , device, epoch, train_sampler, args.max_norm, tf)
        train_log = 'Train() Summary Epoch: [{0}]\tAverage Loss {loss:.3f}\tAverage CER {cer:.3f}%\t'.format(
                    epoch + 1, loss=train_loss, cer=train_cer)
        print(train_log)

        # Test
        test_loss, test_cer = evaluate(args.train_name,model, test_Loader, criterion, device, save_output=True)
        ...
        # Log Summary
        with open("/home/ysy/2022Old_Man_2nd/log/"+args.train_name+"_Summary.txt", "a") as ff:
            ff.write(Lr_inf0 + "\n")
            ff.write(train_log + "\n")
            ff.write(test_log + "\n")
            ff.write(Time_info + "\n\n")
