...
class EncoderRNN(nn.Module):
    def __init__(self, input_size, hidden_size, n_layers=1,
        ...
        """
        기본 LAS시에 bi-direction안쓴는 디코더의 경우 디멘션 낮춰줘야 함. 
        """     
        #self.rnn = nn.Sequential( )# 'Sequential' object has no attribute 'flatten_parameters'
        self.rnn =  self.rnn_cell(rnn_input_dims, self.hidden_size, (self.n_layers -1 ), dropout=self.dropout_p, bidirectional=self.bidirectional, batch_first = True)
        self.rnn_last = self.rnn_cell(int(self.hidden_size*2), int(self.hidden_size/2), 1, dropout=self.dropout_p, bidirectional=self.bidirectional, batch_first = True)
    ...
    def forward(self, input_var, input_lengths=None):
        ...
        """
        #print(x)
        #print("x.size() at RNN",x.size()) # torch.Size([16, 717, 1024]) 
        #print("hidden size",self.hidden_size) # 512 beacuse bi-directional1!
